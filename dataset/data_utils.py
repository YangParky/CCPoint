...
class PointcloudHorizontalFlip(object):
    def __init__(self, upright_axis='z', p=0.5):
        """
        upright_axis: axis index among x,y,z, i.e. 2 for z
        p: probability of performing the flip operation
        """
        ...
        if random.random() < self.p:
            for curr_ax in self.horz_axes:
                if random.random() < 0.5:
                    points_max = torch.max(points[:, curr_ax])
                    points[:, curr_ax] = points_max - points[:, curr_ax]
        return points
